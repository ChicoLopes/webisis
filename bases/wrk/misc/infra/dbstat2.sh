...
# ------------------------------------------------------------------------- #
# chamada: dbstat2 <base_in> [conj_char]
...
# ATENCAO: CISIS DEVE ESTAR EM ``CISIS_DIR''
# ------------------------------------------------------------------------- #
# Data      Resp         Comentario
# 19931121  Renato       Edicao original
# 20050223  fjlopes      Passa a fazer analise de subcampos (mxf0sfld)
#                           qdo o aplicativo estiver excutando corretamente,
#                           ou seja acumulando dados no campo 1020 apenas e
#                           nao no 1021 e no 1020
#
...
export CISIS_DIR=/usr/local/bireme/cisis/4.1r2/lind

if [ "$#" -lt 1 -o "$#" -gt 2 ]
then
  TPR="fatal"
  MSG="use: dbstat2 <dbn> [ascii/ansi/ebcdic/flat]"
  . log
fi
...
. scrmax
TPR="iffatal"
MSG="error: mxf0sfld can not create database"
$CISIS_DIR/mxf0sfld $1 create=stat $MAXMFN noedit
. log
...
echo "'                Tag and Sub-Filed Distribution Analisys - TDA',/,">> 1.pft
echo "'-----------------------------------------------------------------------------',/#,">> 1.pft
...
echo "(">> 1.pft
echo " c2,">> 1.pft
echo " f(val(v1020^t),5,0),\"^\"v1020^s,">> 1.pft
echo " c11,">> 1.pft
echo " f(val(v1020^d),8,0),">> 1.pft
echo " c21,">> 1.pft
echo " f(100*val(v1020^d)/val(v1009[1]),6,1),">> 1.pft
echo " c29,">> 1.pft
echo " f(val(v1020^o),8,0),">> 1.pft
echo " c39,">> 1.pft
...
echo " endsel,">> 1.pft
echo " c42,">> 1.pft
echo " f(val(v1020^l),7,0),">> 1.pft
echo " c51,">> 1.pft
echo " f(val(v1020^u),7,0),">> 1.pft
echo " c60,">> 1.pft
echo " f(val(v1020^n),10,0),/">> 1.pft
echo ")/##">> 1.pft

cp -p 1.pft reserva.pft
# ------------------------------------------------------------------- #
# Gera os arquivos relatorios

TPR="iffatal"
MSG="error: mx can't create report $1.sta with TAG analisys format"
$CISIS_DIR/mx stat pft=@1.pft -all now > $1.sta
. log
