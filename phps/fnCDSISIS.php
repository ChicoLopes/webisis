...
function STATUS ($nomebase, &$CONTROL) {
	...
	//echo "CISISDIR: $cisisDir<br>";
	//echo "CISISTAB: $cisisTab<br>";
	//echo "CISISGIZ: $cisisGiz<br>";
	...
	if (!file_exists($pathbase)) {
		...
		if (! is_writable($pathbase)) { echo "Diretório R/O<br>"; $returnCode |= 32; }	// Nao podendo ajusta codigo de retorno
		...
		//die("tmp: $tmp<br>\n");
		if ($tmp == 0) {
			//die("$cisisDir/mx "   . $database . " +control count=0");
...
function CREATE ($nomebase, $sobrescreve) {
	...
	if ($returnCode == 0) {
		...
		exec("$cisisDir/mx null \"proc='<1 0>criado com qisis</1>'\" create=" . $database . " count=0 \"fst=1 0 v1/\" fullinv=" . $database, $resposta, $retorno);
		// Prepara valores de retorno
		if (isset($resposta[1])) {
 ...
 *			$buffer["v24"][0] => "<An> Electric hygrometer apparatus for measuring water-vapour loss from plants in the field"
