...
<!-- JavaScript para fazer entradas em parte prefixadas nos campos -->
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      // MONOVALORADAS
      document.querySelectorAll(".mono-wrapper").forEach(wrapper => {
        const varName = wrapper.dataset.var;
        const tipo = wrapper.dataset.prefixo;
        //const prefixos = wrapper.dataset.prefixos.split(',');
        const hiddenInput = document.getElementById(varName);
        ...
        wrapper.querySelectorAll('input.part').forEach(input => {
          const pref = input.dataset.prefixo;
          const val = partes.find(p => p.startsWith(pref)) || '';
          input.value = pref ? val.replace(pref, '') : val;
          input.addEventListener('input', () => {
            let novo = '';
            wrapper.querySelectorAll('input.part').forEach((inp, idx) => {
              const pr = inp.dataset.prefixo;
              const v = inp.value.trim();
              if (v !== '') novo += (tipo === 'total' || idx > 0 ? pr : '') + v;
            });
            hiddenInput.value = novo;
          });
        });
      });

      // MULTIVALORADAS
      document.querySelectorAll(".parted-textarea").forEach(textarea => {
        const varName = textarea.dataset.var;
        const tipo = textarea.dataset.prefixo;
        const prefixos = textarea.dataset.prefixos.split(',');  // new.line
        const hidden = document.getElementById(varName);

        const linhas = hidden.value.split('\n').filter(l => l.trim() !== '');
        const convertidas = linhas.map(linha => {
          ...
          // old.line return partes.join('|').replace(/\^./g, '');
          return prefixos.map(p => {        // new.lines ini
            const encontrado = partes.find(part => part.startsWith(p));
            return encontrado ? encontrado.replace(p, '') : '';
          }).join('|');                     // new.line fim
        });

        textarea.value = convertidas.join('\n');

        textarea.addEventListener('input', () => {
          const novo = textarea.value.split('\n').map(l => {
            const partes = l.split('|');
            return partes.map((p, idx) => {
              // old.line const pref = tipo === 'parcial' && idx === 0 ? '' : ['','^n','^p','^d','^z','^v','^a','^b','^c','^z'][idx] || '';
              const pref = prefixos[idx] || '';   // new.line
              return p.trim() !== '' ? pref + p.trim() : '';
            }).join('');
          }).join('\n');
          hidden.value = novo;
        });
